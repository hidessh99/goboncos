<template>
  <div class="p-4 sm:p-6 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-semibold text-indigo-600 dark:text-indigo-400">Tipe Kategori Finansial</h2>
        <p class="text-xs text-gray-500 dark:text-zinc-400 mt-0.5">Kelola tipe kategori untuk pemetaan transaksi finansial</p>
      </div>
    ...
    <div class="bg-white dark:bg-zinc-900/50 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
            ...
            <TableBody>
              <TableRow v-if="isLoading">
                <TableCell colspan="3" class="h-32 text-center text-zinc-500">
                  ...
                  <p class="mt-2 text-sm font-medium">Memuat data tipe kategori...</p>
                </TableCell>
              </TableRow>
              <TableRow v-else-if="types.length === 0">
                <TableCell colspan="3" class="h-32 text-center">
                  <div class="flex flex-col items-center gap-2">
                    ...
                    <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Tidak ada tipe kategori ditemukan.</p>
                  </div>
                </TableCell>
              </TableRow>
              <TableRow 
                ...
                @click="openTypeDetail(type)"
              >
                <TableCell class="py-2.5 pl-4">
                  <div class="flex items-center gap-3">
                    <div :class="`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold text-white shrink-0 shadow-sm ${getAvatarColor(type.name)}`">
                      {{ type.name.charAt(0).toUpperCase() }}
                    </div>
                    <span class="text-[13px] font-semibold text-gray-900 dark:text-gray-100 truncate max-w-[150px] sm:max-w-none">{{ type.name }}</span>
                  </div>
                </TableCell>
                ...
                <TableCell class="py-2.5 pr-4 text-center">
                  <div class="flex items-center justify-center gap-1.5" @click.stop>
                    <button @click="openTypeDetail(type)" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors">
                      <Eye class="w-[15px] h-[15px]" />
                    </button>
                    <button @click="openEditModal(type)" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-amber-50 hover:text-amber-600 transition-colors">
                      <Pencil class="w-[15px] h-[15px]" />
                    </button>
                    <button @click="confirmDelete(type.id)" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-red-50 hover:text-red-500 transition-all">
      ...
      <div class="px-4 py-4 md:hidden flex flex-col gap-4">
        ...
        <div v-else-if="types.length === 0" class="py-12 text-center border border-dashed border-gray-200 dark:border-zinc-800 rounded-2xl">
           <Grid2X2 class="w-10 h-10 text-gray-300 dark:text-zinc-600 mx-auto" />
           <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400 mt-2">Belum ada tipe kategori.</p>
        </div>
        <div 
          v-for="type in types" 
          :key="type.id"
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex items-center gap-3">
              <div :class="`w-10 h-10 rounded-2xl flex items-center justify-center text-xs font-black text-white shrink-0 shadow-md ${getAvatarColor(type.name)}`">
                {{ type.name.charAt(0).toUpperCase() }}
              </div>
              <div class="min-w-0 pr-8">
                <h4 class="text-[13px] font-bold text-gray-900 dark:text-white truncate leading-tight">{{ type.name }}</h4>
                <p class="text-[11px] text-gray-400 mt-0.5 truncate uppercase">Dibuat: {{ formatDate(type.created_at) }}</p>
              </div>
            </div>
            <div @click.stop="openTypeDetail(type)" class="w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600">
               <Eye class="w-[14px] h-[14px]" />
            </div>
          </div>
          <div class="mt-4 flex gap-2.5 pt-4 border-t border-gray-50 dark:border-zinc-800/60" @click.stop>
             <Button variant="outline" class="flex-1 h-10 rounded-xl border-amber-200 text-amber-600 hover:bg-amber-50 text-xs font-bold gap-2" @click="openEditModal(type)">
               <Pencil class="w-3.5 h-3.5" />
               Edit
             </Button>
             <Button variant="outline" class="flex-1 h-10 rounded-xl border-red-200 text-red-500 hover:bg-red-50 text-xs font-bold gap-2" @click="confirmDelete(type.id)">
    ...
    <Dialog :open="isDetailModalOpen" @update:open="isDetailModalOpen = $event">
      <DialogContent class="w-[calc(100%-2rem)] sm:max-w-md bg-white dark:bg-zinc-900 rounded-2xl p-0 overflow-hidden outline-none border-none shadow-2xl">
        <div v-if="selectedType" class="p-8 space-y-6">
          <div class="flex flex-col items-center text-center space-y-4">
            <div :class="`w-20 h-20 rounded-3xl flex items-center justify-center text-3xl font-black text-white shadow-2xl rotate-3 shrink-0 ${getAvatarColor(selectedType.name)}`">
              {{ selectedType.name.charAt(0).toUpperCase() }}
            </div>
            <div>
              <h3 class="text-lg font-black text-gray-900 dark:text-white leading-tight uppercase tracking-tight">{{ selectedType.name }}</h3>
              <p class="text-xs font-mono font-bold text-indigo-500 mt-1 uppercase tracking-widest">ID: {{ selectedType.id.substring(0, 8) }}...</p>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div class="p-5 rounded-3xl bg-gray-50 dark:bg-zinc-800/30 border border-gray-100 dark:border-zinc-800/50">
              ...
              <p class="text-[13px] font-bold text-gray-700 dark:text-zinc-300 text-left">{{ formatDate(selectedType.created_at) }}</p>
            </div>
          </div>
    ...
    <Dialog :open="isFormModalOpen" @update:open="val => { if (!isSubmitting) isFormModalOpen = val }">
      <DialogContent class="w-[calc(100%-2rem)] sm:max-w-md bg-white dark:bg-zinc-900 border-none shadow-2xl rounded-2xl p-0 overflow-hidden outline-none">
        <div class="max-h-[90vh] overflow-y-auto">
          <!-- Header -->
          <div class="flex items-center gap-4 p-5 sm:p-6 border-b border-gray-100 sticky top-0 bg-white backdrop-blur-md z-20">
            ...
            <div class="text-left">
              <DialogTitle class="text-[15px] sm:text-lg font-bold text-gray-900 dark:text-white leading-tight">
                {{ isEditing ? 'Edit Tipe Kategori' : 'Tambah Tipe Kategori' }}
              </DialogTitle>
              <DialogDescription class="text-xs sm:text-sm text-gray-400 mt-0.5">
                Isi form di bawah untuk mengelola tipe kategori finansial.
              </DialogDescription>
            </div>
          </div>
          ...
          <form @submit.prevent="submitForm" class="p-5 sm:p-6 space-y-6">
            <div class="space-y-4 text-left">
              <div class="space-y-2">
                <label class="text-[11px] font-black text-gray-400 uppercase tracking-widest block px-0.5">Nama Tipe <span class="text-red-500">*</span></label>
                <input 
                  v-model="form.name" 
                  type="text" 
                  placeholder="E.g. Transfer, Tunai, Kartu Kredit" 
      ...
      <AlertDialogContent class="bg-white dark:bg-zinc-900 rounded-2xl border-none shadow-2xl p-8 max-w-[90vw] sm:max-w-md">
        <AlertDialogHeader class="space-y-4">
          ...
          <div class="space-y-2 text-center">
            <AlertDialogTitle class="text-xl font-black text-gray-900 dark:text-white">Hapus Tipe?</AlertDialogTitle>
            <AlertDialogDescription class="text-sm font-medium text-gray-500 px-4">Tindakan ini permanen. Pastikan tipe ini memang ingin dihapus.</AlertDialogDescription>
          </div>
        </AlertDialogHeader>
...
import { 
  getCategoryTypes, createCategoryType, updateCategoryType, deleteCategoryType, 
  type CategoryTypeItem 
} from '@/server/api/admin/category-type'
import { alertSuccess, alertError } from '@/lib/alert'

definePageMeta({ layout: 'dashboard' })

const types = ref<CategoryTypeItem[]>([])
...
const selectedType = ref<CategoryTypeItem | null>(null)
...
const fetchTypes = async () => {
  ...
  try {
    ...
    const response = await getCategoryTypes(token, currentPage.value, pageSize.value, searchQuery.value)
    const result = await response.json()
    
    if (response.ok && result.success) {
      types.value = result.payload || []
...
const prevPage = () => { if (currentPage.value > 1) { currentPage.value--; fetchTypes() } }
const nextPage = () => { if (currentPage.value < totalPages.value) { currentPage.value++; fetchTypes() } }

// ─── Handlers ───────────────────────────────────────────────────────────────
const openTypeDetail = (type: CategoryTypeItem) => {
...
const openEditModal = (type: CategoryTypeItem) => {
...
const submitForm = async () => {
  ...
  try {
    ...
    const response = isEditing.value 
      ? await updateCategoryType(selectedType.value!.id, payload, token)
      : await createCategoryType(payload, token)
    
    const result = await response.json()
    if (response.ok && result.success) {
      alertSuccess(isEditing.value ? 'Tipe diperbarui.' : 'Tipe ditambahkan.')
      isFormModalOpen.value = false
      fetchTypes()
    } else {
      alertError(result.message || 'Gagal menyimpan tipe.')
    }
  } catch (error) {
...
const executeDelete = async () => {
  ...
  try {
    const token = localStorage.getItem('token') || ''
    const response = await deleteCategoryType(deleteItemId.value, token)
    const result = await response.json()
    if (response.ok && result.success) {
      alertSuccess('Tipe berhasil dihapus.')
      fetchTypes()
    } else {
      alertError(result.message || 'Gagal menghapus tipe.')
    }
  } catch (error) {
    alertError('Kesalahan jaringan.')
  } finally {
