<template>
  <div class="p-4 sm:p-6 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-semibold text-emerald-600 dark:text-emerald-400">Kategori Investasi</h2>
        <p class="text-xs text-gray-500 dark:text-zinc-400 mt-0.5">Kelola kategori investasi untuk pemetaan portofolio Anda</p>
      </div>
      <div class="flex items-center gap-2">
        <Button 
          id="btn-tambah-kategori"
          class="bg-emerald-600 hover:bg-emerald-700 text-white gap-2 h-11 px-5 rounded-xl shadow-sm shadow-emerald-500/20"
              ...
              >
                ...
                <TableCell class="py-2.5 pr-4 text-center">
                   <div class="flex items-center justify-center gap-1.5" @click.stop>
                     <button @click="openDetail(item)" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors">
                       <Eye class="w-[15px] h-[15px]" />
                     </button>
                     <button @click="openEditModal(item)" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-amber-50 hover:text-amber-600 transition-colors">
                       <Pencil class="w-[15px] h-[15px]" />
                     </button>
                     <button @click="confirmDelete(item.id)" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-red-50 hover:text-red-500 transition-all">
          ...
          <div class="flex items-start justify-between">
            <div class="flex items-center gap-3">
              ...
              <div class="min-w-0 pr-8">
                <h4 class="text-[13px] font-bold text-gray-900 dark:text-white truncate leading-tight">{{ item.name }}</h4>
                <p class="text-[11px] text-gray-400 mt-0.5 truncate uppercase">Dibuat: {{ formatDate(item.created_at) }}</p>
              </div>
            </div>
            <div @click.stop="openDetail(item)" class="w-8 h-8 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-600">
      ...
      <DialogContent class="w-[calc(100%-2rem)] sm:max-w-md bg-white dark:bg-zinc-900 rounded-2xl p-0 overflow-hidden outline-none border-none shadow-2xl">
        <div v-if="selectedItem" class="p-8 space-y-6">
          <div class="flex flex-col items-center text-center space-y-4">
            <div :class="`w-20 h-20 rounded-3xl flex items-center justify-center text-3xl font-black text-white shadow-2xl rotate-3 shrink-0 ${getAvatarColor(selectedItem.name)}`">
              {{ selectedItem.name.charAt(0).toUpperCase() }}
            </div>
            <div>
              <h3 class="text-lg font-black text-gray-900 dark:text-white leading-tight uppercase tracking-tight">{{ selectedItem.name }}</h3>
              <p class="text-xs font-mono font-bold text-emerald-500 mt-1 uppercase tracking-widest">ID: {{ selectedItem.id.substring(0, 8) }}...</p>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div class="p-5 rounded-3xl bg-gray-50 dark:bg-zinc-800/30 border border-gray-100 dark:border-zinc-800/50">
              ...
              <p class="text-[13px] font-bold text-gray-700 dark:text-zinc-300 text-left">{{ formatDate(selectedItem.created_at) }}</p>
            </div>
          </div>
        ...
        <div class="max-h-[90vh] overflow-y-auto">
          ...
          <div class="flex items-center gap-4 p-5 sm:p-6 border-b border-gray-100 sticky top-0 bg-white backdrop-blur-md z-20">
            ...
            <div class="text-left">
              <DialogTitle class="text-[15px] sm:text-lg font-bold text-gray-900 dark:text-white leading-tight">
                {{ isEditing ? 'Edit Kategori Investasi' : 'Tambah Kategori Investasi' }}
              </DialogTitle>
              <DialogDescription class="text-xs sm:text-sm text-gray-400 mt-0.5">
                Isi form di bawah untuk mengelola kategori investasi Anda.
              </DialogDescription>
            </div>
          </div>
          ...
          <form @submit.prevent="submitForm" class="p-5 sm:p-6 space-y-6">
            <div class="space-y-4 text-left">
              <div class="space-y-2">
                ...
                <input 
                  v-model="form.name" 
                  type="text" 
                  placeholder="E.g. Saham, Reksa Dana, Kripto" 
                  class="w-full px-4 py-3 rounded-xl border border-gray-200 text-[13px] font-semibold outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all shadow-sm" 
                  required
                />
              </div>
            </div>
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-50">
              ...
              <Button type="submit" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl py-6 font-bold text-sm" :disabled="isSubmitting || !form.name.trim()">
      ...
      <AlertDialogContent class="bg-white dark:bg-zinc-900 rounded-2xl border-none shadow-2xl p-8 max-w-[90vw] sm:max-w-md">
        <AlertDialogHeader class="space-y-4">
          ...
          <div class="space-y-2 text-center">
            ...
            <AlertDialogDescription class="text-sm font-medium text-gray-500 px-4">Tindakan ini permanen. Pastikan kategori ini memang ingin dihapus.</AlertDialogDescription>
          </div>
        </AlertDialogHeader>
...
import { 
  getInvestmentCategories, createInvestmentCategory, updateInvestmentCategory, deleteInvestmentCategory, 
  type InvestmentCategoryItem 
} from '@/server/api/admin/investment-category'
import { alertSuccess, alertError } from '@/lib/alert'

definePageMeta({ layout: 'dashboard' })

useSeoMeta({
  title: 'Admin - Kategori Investasi',
  robots: 'noindex, nofollow'
})

const categories = ref<InvestmentCategoryItem[]>([])
...
const selectedItem = ref<InvestmentCategoryItem | null>(null)
...
const fetchCategories = async () => {
  ...
  try {
    ...
    const response = await getInvestmentCategories(token, currentPage.value, pageSize.value, searchQuery.value)
...
// ─── Handlers ───────────────────────────────────────────────────────────────
const openDetail = (item: InvestmentCategoryItem) => {
...
const openEditModal = (item: InvestmentCategoryItem) => {
...
const submitForm = async () => {
  ...
  try {
    ...
    const response = isEditing.value 
      ? await updateInvestmentCategory(selectedItem.value!.id, payload, token)
      : await createInvestmentCategory(payload, token)
...
const executeDelete = async () => {
  ...
  try {
    const token = localStorage.getItem('token') || ''
    const response = await deleteInvestmentCategory(deleteItemId.value, token)
    const result = await response.json()
    if (response.ok && result.success) {
